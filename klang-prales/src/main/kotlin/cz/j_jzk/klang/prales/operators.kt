...
fun <E> operators(expr: NodeID<E>) = lesana<Oper<E>> {
    // from highest priority to lowest priority
    // ARITHMETIC OPERATIONS
    val lvl1 = NodeID<Oper<E>>()
    lvl1 to def(expr) { Oper.Id(it.v1) }

    val lvl2 = NodeID<Oper<E>>()
    ...
    // LOGIC OPERATIONS
    val lvl4 = NodeID<Oper<E>>()
    lvl4 to def(re("!"), lvl3) { (_, a) -> Oper.Log.Not(a) }
    lvl4 to def(lvl3) { it.v1 }

    val lvl5 = NodeID<Oper<E>>()
    lvl5 to def(lvl5, re("&&"), lvl4) { (a, _, b) -> Oper.Log.And(a, b) }
    lvl5 to def(lvl4) { it.v1 }

    val lvl6 = NodeID<Oper<E>>()
    lvl6 to def(lvl6, re("\\|\\|"), lvl5) { (a, _, b) -> Oper.Log.Or(a, b) }
    lvl6 to def(lvl5) { it.v1 }

    lvl1 to def(re("\\("), lvl6, re("\\)")) { it.v2 }

    val top = NodeID<Oper<E>>()
    top to def(lvl6) { it.v1 }
    setTopNode(top)
    inheritIgnoredREs()
}
